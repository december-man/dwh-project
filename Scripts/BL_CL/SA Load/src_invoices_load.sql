CREATE OR REPLACE PROCEDURE BL_CL.SRC_INVOICES_RELOAD_CSV(IN file_abspath TEXT) AS 
$src_invoices_loader$
DECLARE
...
BEGIN
	...
	DROP TABLE IF EXISTS SA_SALES_INVOICES.SRC_INVOICES;
	CREATE TABLE SA_SALES_INVOICES.SRC_INVOICES AS TABLE SA_SALES_INVOICES.EXT_INVOICES;
	...
	PERFORM * FROM sa_sales_invoices.src_invoices; -- get row count in load logs
	-- Logging
	GET DIAGNOSTICS 
		context := PG_CONTEXT,
		row_count := ROW_COUNT;
