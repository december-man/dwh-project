CREATE OR REPLACE PROCEDURE BL_CL.SRC_CUSTOMERS_CC_RELOAD_CSV(IN file_abspath TEXT) AS 
$src_ccc_loader$
DECLARE
...
BEGIN
	...
	DROP TABLE IF EXISTS SA_SALES_CUSTOMERS_CC.SRC_CUSTOMERS_CC;
	CREATE TABLE SA_SALES_CUSTOMERS_CC.SRC_CUSTOMERS_CC AS TABLE SA_SALES_CUSTOMERS_CC.EXT_CUSTOMERS_CC;
	...
	PERFORM * FROM sa_sales_customers_cc.src_customers_cc; -- get row count in load logs
	-- Logging
	GET DIAGNOSTICS 
		context := PG_CONTEXT,
		row_count := ROW_COUNT;
